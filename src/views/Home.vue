...
const startLottery = async () => {
  ...
  try {
    ...
    // 打乱参与者列表
    participants.value = shuffleArray(availableParticipantsList)
    console.log('参与者列表已打乱')
    ...
    if (namesContainer.value) {
      scrollInterval.value = setInterval(() => {
        rollingOffset.value += 40; // 调整滚动速度，确保与减速动画更好衔接

        if (namesContainer.value) {
          // 清除任何可能存在的transition，确保快速滚动时没有过渡效果
          namesContainer.value.style.transition = 'none';
...
const stopLottery = async () => {
  ...
  try {
    ...
    const currentPosition = Math.floor(rollingOffset.value / itemHeight)
    const additionalScrollSteps = 12 // 再滚动5-12个位置
    const targetStopPosition = currentPosition + additionalScrollSteps

    // 动态创建最终滚动列表，确保中奖者出现在目标位置

    const targetIndex = targetStopPosition % participants.length
    console.log('目标人员:', winnerName)
    // 在目标位置插入真实中奖者
    const winnerParticipant = participants.value.find(p => p.name === winnerName)
    if (winnerParticipant) {
      participants[targetIndex] = winnerParticipant
    }
    // 更新显示列表 - 修复：确保更新完整的参与者列表
    // const dynamicScrollList = createDynamicScrollList()
    // console.log('动态滚动列表:', dynamicScrollList)
    // participants.value = dynamicScrollList.slice(0, dynamicScrollList.length / 2) // 取前半部分作为基础列表

    // 保存减速开始时的位置，确保无缝衔接
    const startOffset = rollingOffset.value
    const targetOffset = targetStopPosition * itemHeight

    // 老虎机减速效果：逐渐减速到目标位置
    let slowDownSteps = 0
    const maxSlowDownSteps = 20 // 增加减速步数，使动画更平滑

    const slowDownInterval = setInterval(() => {
      slowDownSteps++

      // 计算减速曲线 - 使用更平滑的缓出函数
      const progress = slowDownSteps / maxSlowDownSteps
      const easeOut = 1 - Math.pow(1 - progress, 4) // 更平滑的缓出曲线

      // 计算当前应该到达的位置（基于固定的起始位置）
      const currentTargetOffset = startOffset + (targetOffset - startOffset) * easeOut

      // 更新滚动位置
      rollingOffset.value = currentTargetOffset
      ...
      if (slowDownSteps >= maxSlowDownSteps) {
        ...
        if (namesContainer.value) {
          // 最终精确定位到中奖者 - 确保无缝衔接
          const finalWinnerIndex = participants.value.findIndex(p => p.name === winnerName)
          if (finalWinnerIndex !== -1) {
            const slotHeight = 220 // 老虎机容器高度
            const centerOffset = (slotHeight - itemHeight) / 2 // 居中偏移量
            const finalTargetOffset = finalWinnerIndex * itemHeight - centerOffset

            // 检查当前位置与最终位置的差距
            const currentOffset = rollingOffset.value
            const offsetDifference = Math.abs(finalTargetOffset - currentOffset)

            // 如果差距很小，直接设置最终位置；如果差距较大，使用平滑过渡
            if (offsetDifference < itemHeight / 4) {
              // 差距很小，直接设置
              namesContainer.value.style.transition = 'none'
              namesContainer.value.style.transform = `translateY(-${finalTargetOffset}px)`
              rollingOffset.value = finalTargetOffset
            } else {
              // 差距较大，使用平滑过渡
              namesContainer.value.style.transition = 'all 0.2s cubic-bezier(0.25, 0.46, 0.45, 0.94)'
              namesContainer.value.style.transform = `translateY(-${finalTargetOffset}px)`
              rollingOffset.value = finalTargetOffset
            }
            ...
            setTimeout(() => {
              ...
              // 基于当前滚动位置计算高亮索引，确保与视觉位置一致
              const currentScrollPosition = Math.round(rollingOffset.value / itemHeight)
              const slotVisibleIndex = currentScrollPosition + Math.floor((220 - itemHeight) / 2 / itemHeight) // 计算老虎机中心位置对应的索引

              nameElements.forEach((name, index) => {
                name.classList.remove('selected', 'winner-pulse')
                if (index === slotVisibleIndex) {
                  name.classList.add('selected')
                  name.classList.add('winner-pulse')
                }
              })

              // 清除transition以避免后续操作的影响
              if (namesContainer.value) {
                namesContainer.value.style.transition = 'none'
              }
            ...
            }, offsetDifference < itemHeight / 4 ? 100 : 200) // 根据是否有过渡调整延迟时间
          } else {
            // 如果找不到中奖者，直接结束
            isSlotRunning.value = false
            handleLotteryResultWithData(drawnWinners, updated_awards)
